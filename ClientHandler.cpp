...
int ClientHandler::handleGraph(int fd) {
    ClientHandler::outputHandler(CLEAR_TERMINAL + "Requesting Permission to Graph...\n",fd);
    MainGraph* graph = MainGraph::getInstance();
    MainGraph::lockInstance();
    string cmd = ClientHandler::inputHandler(CLEAR_TERMINAL + "Write a Command:"
    ...
    int exit = 0;
    if (cmd == "Newgraph") {
        string number = ClientHandler::inputHandler(CLEAR_TERMINAL + "Choose graph size\nYour Input: ", fd);
        try {
            int size = stoi(number);
            graph->newGraph(size);
        }
        catch (const std::invalid_argument& e) {
            ClientHandler::outputHandler(CLEAR_TERMINAL + "Number not provided!",fd);
            sleep(1);
        }
    }
    else if (cmd == "MST") {
        ...
        string input = ClientHandler::inputHandler(CLEAR_TERMINAL + "Add Edge:\n[format: v u w]\nYour Input: ",fd);
        ...
        string input = ClientHandler::inputHandler(CLEAR_TERMINAL + "Remove Edge:\n[format: v u]\nYour Input: ",fd);
        stringstream inputStream(input);
        string vstr,ustr;
        inputStream >> vstr;
        inputStream >> ustr;
        ...
        ClientHandler::outputHandler(CLEAR_TERMINAL + "Unknown command!",fd);
        sleep(1);
    }
